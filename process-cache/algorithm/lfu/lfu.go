...
/**
LFU（最少使用）算法会淘汰缓存中访问次数最少的数据。在LFU设计中，其核心原则是，如果数据过去被访问多次，那么将来被访问的频率也会更高。
在LFU的实现中，需要维护一个按照访问次数排序的队列，每次访问时，访问次数都加 1，队列重新排序，淘汰时选择访问次数最少的即可
*/
...
func New(maxBytes int, onEvicted func(key string, value interface{})) cache.ICache {
	q := make(queue, 0, 1024)
	return &lfu{
		maxBytes:  maxBytes,
		onEvicted: onEvicted,
